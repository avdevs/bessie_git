...
from bessie.models import Employee, Company, CompanyAdmin
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if company.slots < row_count:
                messages.error(request, f'You don\'t have enough slots to invite {row_count} employees.')
            ...
            data_rows = [data for data in data_rows if len(data) >= MIN_COLUMNS and data[2] not in existing_emails]
            ...
            for data in data_rows:
                try:
                    ...
                    html_message = render_to_string('emails/employee_invitation_email.html', {
                        'user': user,
                        'password': password,
                        'login_url': login_url,
                        'company': company
            ...
            messages.success(request, f'File uploaded successfully. {len(data_rows)} rows found.')
            return redirect('dashboard')
