...
from ..forms import get_choice_score
...
class BessieQuestionaireWizard(LoginRequiredMixin, SessionWizardView):
	...
	def _sum_question_scores(self, form_data, question_keys):
		...
		for key in question_keys:
			...
			# Handle both old numeric values and new choice identifiers
	...
	def done(self, form_list, **kwargs):
		...
		q63_response = self._get_question_score(form_data, "q63", 0)
		q97_response = self._get_question_score(form_data, "q97", 0)
		q112_response = self._get_question_score(form_data, "q112", 0)
		q121_response = self._get_question_score(form_data, "q121", 0)
		q132_response = self._get_question_score(form_data, "q132", 0)
		q140_response = self._get_question_score(form_data, "q140", 0)
		q156_response = self._get_question_score(form_data, "q156", 0)
		q225_response = self._get_question_score(form_data, "q225", 0)
		...
		for question_key in environment_questions:
			total_environment_score += self._get_question_score(form_data, question_key, 0)
		...
		results.privacy = self._calculate_percentage(total_privacy_score, max_privacy_score)
		...
		total_residence_score = self._sum_question_scores(form_data, residence_questions)
		results.residence = self._calculate_percentage(
			total_residence_score, max_residence_score
		)

		# ===== PAY CALCULATIONS =====
		potential_cost = calc_potential_cost(self._get_question_score(form_data, "q17", 0))
		results.potential_cost = 0.0 if potential_cost is None else float(potential_cost)
		...
		total_childcare_score = self._sum_question_scores(form_data, childcare_questions)
		...
		total_complexity_score = self._sum_question_scores(form_data, complexity_questions)
		...
		control_autonomy_score = self._get_question_score(form_data, "q25", 0)
		...
		# Workload
		workload_score = self._get_question_score(form_data, "q29", 0)
		max_workload_score = 6
		results.workload = self._calculate_percentage(workload_score, max_workload_score)
		...
		# Work Breaks
		work_breaks_score = self._get_question_score(form_data, "q28", 0)
		...
		# Overtime
		overtime_score = self._get_question_score(form_data, "q30", 0)
		max_overtime_score = 6
		results.overtime = self._calculate_percentage(overtime_score, max_overtime_score)
		...
		results.culture = self._calculate_percentage(total_culture_score, max_culture_score)
		...
		# Managerial Trust
		managerial_trust_score = self._get_question_score(form_data, "q50", 0)
		...
		# Colleague Trust
		colleague_trust_score = self._get_question_score(form_data, "q51", 0)
		...
		# Consultation
		consultation_score = self._get_question_score(form_data, "q44", 0)
		...
		# Team Morale
		team_morale_score = self._get_question_score(form_data, "q52", 0)
		...
		willingness_to_progress_score = self._get_question_score(form_data, "q48", 0)
		...
		# Lone Working
		lone_working_score = self._get_question_score(form_data, "q49", 0)
		...
		# Wider Team Support
		wider_team_support_score = self._get_question_score(form_data, "q53", 0)
		...
		results.lone_working_and_wider_team_support_multiplier = self._calculate_percentage(
			lone_wider_support_multiplier_score,
		...
		results.absence = self._calculate_percentage(total_absence_score, max_absence_score)

		# Absence and Support
		q59_response = self._get_question_score(form_data, "q59", 0)
		...
		sick_leave_support_score = self._get_question_score(form_data, "q59", 0)
		...
		# ===== COVID CALCULATIONS =====
		covid_score = self._get_question_score(
			form_data, "q60", 0
		) + self._get_question_score(form_data, "q61", 0)
		max_covid_score = 8
		...
		results.work_related_stress_impacting_personal_life = self._calculate_percentage(
			work_stress_personal_life_score, max_work_stress_personal_life_score
		)

		# Get Q62, Q64 responses
		q62_response = self._get_question_score(form_data, "q62", 0)
		q64_response = self._get_question_score(form_data, "q64", 0)
		...
		results.work_related_stress_impacting_family_and_relationships = (
			self._calculate_percentage(work_stress_family_score, max_work_stress_family_score)
		)
		...
		# ===== CARER CALCULATION =====
		carer_score = self._get_question_score(
			form_data, "q83", 0
		) + self._get_question_score(form_data, "q198", 0)
		max_carer_score = 6
		...
		personal_satisfaction_score = self._get_question_score(form_data, "q202", 0)
		...
		personal_satisfaction_improvement_score = (
			personal_satisfaction_score + self._get_question_score(form_data, "q203", 0)
		)
		...
		results.personal_finances_and_responsibility_for_children = (
			self._calculate_percentage(finances_children_score, max_finances_children_score)
		)
		...
		financial_position_barrier_score = self._get_question_score(form_data, "q199", 0)
		max_financial_position_barrier_score = 6
		results.financial_position_as_a_barrier_for_holidays = self._calculate_percentage(
			financial_position_barrier_score, max_financial_position_barrier_score
		)

		# Work Commitments as a Barrier for Holidays
		work_commitment_barrier_for_holidays_score = self._get_question_score(
			form_data, "q200", 0
		)
		...
		for disc_type, (q_key, max_score) in discrimination_types.items():
			disc_score = self._get_question_score(form_data, q_key, 0) * q225_response
			setattr(
				results,
		...
		if total_personal_relationships_score > 0:
			personal_work_relationships_impact_score = (
				total_personal_relationships_score
				+ self._get_question_score(form_data, "q179", 0)
			)
		...
		total_fertility_score = self._sum_question_scores(form_data, fertility_questions)
		...
		fertility_pregnancy_work_score = self._get_question_score(form_data, "q132", 0)
		...
		if total_fertility_score > 0:
			pregnancy_mental_health_score = total_fertility_score + total_mental_health_score
		else:
		...
		pregnancy_management_support_percentage = (
			(pregnancy_management_support_score / max_pregnancy_management_support_score)
			* 100
		...
		abuse_trauma_impact_score = (
			total_abuse_trauma_score * q140_response if total_abuse_trauma_score > 0 else 0
		)
		...
		total_self_care_score = self._sum_question_scores(form_data, self_care_questions)
		...
		physical_health_factors_impacting_work_score = self._get_question_score(
			form_data, "q156", 0
		)
		...
		physical_health_condition_affecting_work_score = self._get_question_score(
			form_data, "q112", 0
		)
		...
		mental_health_factors_impacting_work_score = self._get_question_score(
			form_data, "q121", 0
		)
		...
		for question_key in health_questions:
			total_health_score += self._get_question_score(form_data, question_key, 0)

		results.health = self._calculate_percentage(total_health_score, max_health_score)
		...
		for question_key in personal_questions:
			total_personal_score += self._get_question_score(form_data, question_key, 0)
		...
		hobbies_score = self._get_question_score(form_data, "q188", 0)
		max_hobbies_score = 2
		...
		if total_mental_health_score + total_physical_health_score > 0:
			mental_physical_culture_multiplier_score = (
				total_mental_health_score + total_physical_health_score + total_culture_score
			) * q63_response
		...
		for question_key in family_questions:
			total_family_score += self._get_question_score(form_data, question_key, 0)

		results.family = self._calculate_percentage(total_family_score, max_family_score)
		...
		total_support_network_score = sum(
			self._get_question_score(form_data, question_key, 0)
			for question_key in support_network
		)
