...
def index(request):
    match request.user.user_type:
        ...
        case "EMPLOYEE":
            ...
            return render(request, 'bessie/index.html', {'response': response, 'employee': employee})
        ...
        case "COMPANY_ADMIN":
            ...
            results_ready = employees.count() == len(list(filter(lambda x: x.has_response == True, employees)))
            form = BulkUserInviteForm()
            form1 = AdminInviteForm(initial={'company_id': comp_admin.company.id})
            return render(request, 'bessie/comp_admin_home.html', {
    ...
    def form_valid(self, form):
        try:
            with transaction.atomic():  
                ...
                user = User(first_name=form.cleaned_data['first_name'], last_name=form.cleaned_data['last_name'], email=form.cleaned_data['email'], user_type=User.UserTypes.COMPANY_ADMIN)
            ...
            if str(e) == "UNIQUE constraint failed: users_user.email":
                messages.error(self.request, f"user with email: {form.cleaned_data['email']} already exists in the system")
            elif str(e) == "UNIQUE constraint failed: bessie_company.name":
                messages.error(self.request, f"company with name: {form.cleaned_data['name']} already exists in the system")
        ...
        html_message = render_to_string('emails/admin_invitation_email.html', {
                        'user': user,
                        'password': password,
                        'login_url': login_url,
                        'company': company
        ...
        send_mail(subject=subject, message=plain_message, from_email='no-reply@bessietech.com', recipient_list=[form.cleaned_data['email']], html_message=html_message)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request, f"user with email: {form.cleaned_data['email']} already exists in the system")
...
def companyDetail(request, id):
    ...
    return render(request, "bessie/company.html", {"company": company, "quizzes": quizzes, "company_admins": company_admins, "employees": employees, "available_slots": company.slots - company.used_slots, "results_ready": results_ready})    
...
class BessieQuizWizard(LoginRequiredMixin, SessionWizardView):
    ...
    def done(self, form_list, **kwargs):
        ...
        response = BessieResponse(employee=employee, 
        ...
        for question_key in environment_questions:
            if form_data.get(question_key) != None and form_data.get(question_key).isdigit() :
        ...
        environment_multiplier_percentage = (environment_multiplier_score / max_environment_multiplier_score) * 100 if max_environment_multiplier_score else 0
       ...
       # Pay Multiplier calculation
        ...
        pay_multiplier_percentage = (
            ...
            if max_pay_multiplier_score else 0
        )
        ...
        childcare_percentage = (total_childcare_score / max_childcare_score) * 100 if max_childcare_score else 0
        ...
        if total_pay_score == 0 or total_childcare_score == 0:
            ...
            pay_childcare_multiplier_score = (total_pay_score + total_childcare_score) * q97_response
        ...
        pay_childcare_multiplier_percentage = (
            ...
            if max_pay_childcare_multiplier_score else 0
        )
        results.pay_and_childcare_multiplier = round(pay_childcare_multiplier_percentage, 2)
       ...
       # Complexity calculation
        ...
        total_hours_flexibility_score = sum(
            int(form_data.get(question_key, 0)) for question_key in hours_flexibility_questions
        )
        hours_flexibility_percentage = (total_hours_flexibility_score / max_hours_flexibility_score) * 100
        ...
        control_autonomy_percentage = (control_autonomy_score / max_control_autonomy_score) * 100 if max_control_autonomy_score else 0
        results.control_and_autonomy_over_working_hours = round(control_autonomy_percentage, 2)
        ...
        total_manageable_workload_score = sum(
            int(form_data.get(question_key, 0)) for question_key in manageable_workload_questions
        )
        manageable_workload_percentage = (total_manageable_workload_score / max_manageable_workload_score) * 100
        ...
        training_percentage = (total_training_score / max_training_score) * 100 if max_training_score else 0
       ...
       # Complexity plus Training calculation
        if total_complexity_score == 0:
            ...
            complexity_plus_training_score = total_complexity_score + total_training_score
        ...
        complexity_plus_training_percentage = (
            ...
            if max_complexity_plus_training_score else 0
        )
        ...
        if total_complexity_score == 0:
            ...
            complexity_training_hours_flexibility_score = (
                total_complexity_score + total_training_score + total_hours_flexibility_score
            )
        max_complexity_training_hours_flexibility_score = 102
        complexity_training_hours_flexibility_percentage = (
            (complexity_training_hours_flexibility_score / max_complexity_training_hours_flexibility_score) * 100
            if max_complexity_training_hours_flexibility_score else 0
        )
        results.complexity_training_hours_and_flexibility = round(complexity_training_hours_flexibility_percentage, 2)

        # Work Breaks calculation
        work_breaks_score = int(form_data.get('q28', 0))
        max_work_breaks_score = 6
        work_breaks_percentage = (work_breaks_score / max_work_breaks_score) * 100 if max_work_breaks_score else 0
        results.work_breaks = round(work_breaks_percentage, 2)

        # Overtime calculation
        overtime_score = int(form_data.get('q30', 0))
        max_overtime_score = 6
        overtime_percentage = (overtime_score / max_overtime_score) * 100 if max_overtime_score else 0
       ...
       # Team and Colleague Support calculation
        ...
        total_team_support_score = sum(
            int(form_data.get(question_key, 0)) for question_key in team_support_questions
        )
        team_support_percentage = (total_team_support_score / max_team_support_score) * 100
        ...
        total_management_support_score = sum(
            int(form_data.get(question_key, 0)) for question_key in management_support_questions
        )
        management_support_percentage = (total_management_support_score / max_management_support_score) * 100
        results.management_support = round(management_support_percentage, 2)

        # Management Support Multiplier calculation
        management_support_multiplier_score = total_management_support_score * q63_response
        max_management_support_multiplier_score = 144
        management_support_multiplier_percentage = (
            (management_support_multiplier_score / max_management_support_multiplier_score) * 100
            if max_management_support_multiplier_score else 0
        )
        results.management_support_multiplier = round(management_support_multiplier_percentage, 2)
        ...
        culture_multiplier_percentage = (
            ...
            if max_culture_multiplier_score else 0
        )
        ...
        total_problem_solving_score = sum(
            int(form_data.get(question_key, 0)) for question_key in problem_solving_questions
        )
        max_problem_solving_score = 12
        problem_solving_percentage = (total_problem_solving_score / max_problem_solving_score) * 100
       ...
       # Managerial Trust calculation
        ...
        managerial_trust_percentage = (managerial_trust_score / max_managerial_trust_score) * 100 if max_managerial_trust_score else 0
        ...
        colleague_trust_percentage = (colleague_trust_score / max_colleague_trust_score) * 100 if max_colleague_trust_score else 0
        ...
        consultation_percentage = (consultation_score / max_consultation_score) * 100 if max_consultation_score else 0
        ...
        team_morale_percentage = (team_morale_score / max_team_morale_score) * 100 if max_team_morale_score else 0
        ...
        willingness_to_progress_percentage = (willingness_to_progress_score / max_willingness_to_progress_score) * 100 if max_willingness_to_progress_score else 0
        ...
        lone_working_percentage = (lone_working_score / max_lone_working_score) * 100 if max_lone_working_score else 0
        ...
        wider_team_support_percentage = (wider_team_support_score / max_wider_team_support_score) * 100 if max_wider_team_support_score else 0
        ...
        if lone_working_score == 0:
            ...
            lone_wider_support_multiplier_score = (lone_working_score + wider_team_support_score) * q63_response
        max_lone_wider_support_multiplier_score = 48
        lone_wider_support_multiplier_percentage = (
            (lone_wider_support_multiplier_score / max_lone_wider_support_multiplier_score) * 100
            if max_lone_wider_support_multiplier_score else 0
        )
        results.lone_working_and_wider_team_support_multiplier = round(lone_wider_support_multiplier_percentage, 2)

        # Team and Colleague Support Multiplier calculation
        team_colleague_support_multiplier_score = total_team_support_score * q63_response
        max_team_colleague_support_multiplier_score = 120
        team_colleague_support_multiplier_percentage = (
            (team_colleague_support_multiplier_score / max_team_colleague_support_multiplier_score) * 100
            if max_team_colleague_support_multiplier_score else 0
        )
        results.team_and_colleague_support_multiplier = round(team_colleague_support_multiplier_percentage, 2)

        # Health and Safety calculation
        health_safety_questions = ['q54', 'q55', 'q56']
        max_health_safety_score = 18
        total_health_safety_score = sum(
            int(form_data.get(question_key, 0)) for question_key in health_safety_questions
        )
        health_safety_percentage = (total_health_safety_score / max_health_safety_score) * 100
    ...
    # Health and Safety Multiplier calculation
        ...
        health_safety_multiplier_percentage = (
            ...
            if max_health_safety_multiplier_score else 0
        )
        results.health_and_safety_multiplier = round(health_safety_multiplier_percentage, 2)
    ...
    # Absence and Support calculation
        ...
        absence_and_support_score = total_absence_score + q59_response if total_absence_score > 0 else 0
    ...
    # Sick Leave and Employer Support calculation
        ...
        sick_leave_support_percentage = (sick_leave_support_score / max_sick_leave_support_score) * 100 if max_sick_leave_support_score else 0
        results.sick_leave_and_employer_support = round(sick_leave_support_percentage, 2)
    ...
    # Covid Multiplier calculation
        ...
        covid_multiplier_percentage = (
            ...
            if max_covid_multiplier_score else 0
        )
    ...
    # Work-Related Stress Impacting personal life calculation
        ...
        work_stress_personal_life_percentage = (
            (work_stress_personal_life_score / max_work_stress_personal_life_score) * 100
            if max_work_stress_personal_life_score else 0
        )
        results.work_related_stress_impacting_personal_life = round(work_stress_personal_life_percentage, 2)
    ...
    # Work-Related Stress Impacting Family and Relationships calculation
        ...
        work_stress_family_percentage = (work_stress_family_score / max_work_stress_family_score) * 100
        results.work_related_stress_impacting_family_and_relationships = round(work_stress_family_percentage, 2)
    ...
    # Work-Related Stress Impacting Social Life calculation
        ...
        work_stress_social_life_percentage = (
            ...
            if max_work_stress_social_life_score else 0
        )
        results.work_related_stress_impacting_social_life = round(work_stress_social_life_percentage, 2)

    # Mental Health calculation
        mental_health_questions = ['q114', 'q115', 'q116', 'q117', 'q118', 'q119', 'q120', 'q121', 'q122']
        max_mental_health_score = 30
        total_mental_health_score = sum(
            int(form_data.get(question_key, 0)) for question_key in mental_health_questions
        )
        mental_health_percentage = (total_mental_health_score / max_mental_health_score) * 100
        ...
        mental_health_multiplier_percentage = (
            ...
            if max_mental_health_multiplier_score else 0
        )
        ...
        physical_health_questions = ['q104', 'q105', 'q106', 'q107', 'q108', 'q109', 'q110', 'q111', 'q112', 'q113']
        max_physical_health_score = 28
        total_physical_health_score = sum(
            int(form_data.get(question_key, 0)) for question_key in physical_health_questions
        )
        physical_health_percentage = (total_physical_health_score / max_physical_health_score) * 100
        ...
        physical_health_multiplier_percentage = (
            (physical_health_multiplier_score / max_physical_health_multiplier_score) * 100
            if max_physical_health_multiplier_score else 0
        )
        results.physical_health_multiplier = round(physical_health_multiplier_percentage, 2)
    
    # Physical Health and Absence Multiplier calculation
        if total_absence_score > 0:
            physical_absence_multiplier_score = (total_physical_health_score + total_absence_score) * q112_response
        else:
            physical_absence_multiplier_score = 0
        max_physical_absence_multiplier_score = 160
        physical_absence_multiplier_percentage = (
            (physical_absence_multiplier_score / max_physical_absence_multiplier_score) * 100
            if max_physical_absence_multiplier_score else 0
        )
        results.physical_health_and_absence_multiplier = round(physical_absence_multiplier_percentage, 2)

    # Mental and Physical Health calculation
        mental_physical_health_score = total_mental_health_score + total_physical_health_score
        ...
        mental_physical_health_percentage = (
            ...
            if max_mental_physical_health_score else 0
        )
    ...
    # Mental and Physical Health Multiplier calculation
        mental_physical_health_multiplier_score = mental_physical_health_score * q156_response
        max_mental_physical_health_multiplier_score = 232
        mental_physical_health_multiplier_percentage = (
            (mental_physical_health_multiplier_score / max_mental_physical_health_multiplier_score) * 100
            if max_mental_physical_health_multiplier_score else 0
        )
        results.mental_and_physical_health_multiplier = round(mental_physical_health_multiplier_percentage, 2)
    ...
    # Family Support Companion calculation
        ...
        total_family_support_score = sum(
            int(form_data.get(question_key, 0)) for question_key in family_support_questions
        )
        family_support_percentage = (total_family_support_score / max_family_support_score) * 100
    ...
    # Responsibility of Children calculation
        ...
        total_responsibility_children_score = sum(
            int(form_data.get(question_key, 0)) for question_key in responsibility_children_questions
        )
        responsibility_children_percentage = (
            (total_responsibility_children_score / max_responsibility_children_score) * 100
        )
        results.responsibility_of_children = round(responsibility_children_percentage, 2)

    # Responsibility for Children Multiplier calculation
        responsibility_children_multiplier_score = total_responsibility_children_score * q97_response
        max_responsibility_children_multiplier_score = 68
        responsibility_children_multiplier_percentage = (
            (responsibility_children_multiplier_score / max_responsibility_children_multiplier_score) * 100
            if max_responsibility_children_multiplier_score else 0
        )
        results.responsibility_for_children_multiplier = round(responsibility_children_multiplier_percentage, 2)
    ...
    # Childcare Multiplier calculation
        ...
        childcare_multiplier_percentage = (
            ...
            if max_childcare_multiplier_score else 0
        )
    ...
    # Emotional Health calculation
        ...
        total_emotional_health_score = sum(
            int(form_data.get(question_key, 0)) for question_key in emotional_health_questions
        )
        emotional_health_percentage = (total_emotional_health_score / max_emotional_health_score) * 100
    ...
    # Personal Satisfaction calculation
        ...
        personal_satisfaction_percentage = (
            ...
            if max_personal_satisfaction_score else 0
        )
        results.personal_satisfaction = round(personal_satisfaction_percentage, 2)

    # Personal Satisfaction Improvement calculation
        personal_satisfaction_improvement_score = personal_satisfaction_score + int(form_data.get('q203', 0))
        max_personal_satisfaction_improvement_score = 12
        personal_satisfaction_improvement_percentage = (
            (personal_satisfaction_improvement_score / max_personal_satisfaction_improvement_score) * 100
            if max_personal_satisfaction_improvement_score else 0
        )
        results.personal_satisfaction_improvement = round(personal_satisfaction_improvement_percentage, 2)
    ...
    # Personal Finances calculation
        ...
        total_personal_finances_score = sum(
            int(form_data.get(question_key, 0)) for question_key in personal_finances_questions
        )
        personal_finances_percentage = (
            (total_personal_finances_score / max_personal_finances_score) * 100
            if max_personal_finances_score else 0
        )
    ...
    # Personal Finances and Responsibility for Children calculation
        if total_personal_finances_score > 0:
            finances_children_score = total_personal_finances_score + total_responsibility_children_score
        else:
        ...
        finances_children_percentage = (
            ...
            if max_finances_children_score else 0
        )
        results.personal_finances_and_responsibility_for_children = round(finances_children_percentage, 2)

    # Personal Finances and Pay Multiplier calculation
        if total_personal_finances_score > 0:
            finances_pay_multiplier_score = total_personal_finances_score * pay_multiplier_score
        else:
        ...
        finances_pay_multiplier_percentage = (
            ...
            if max_finances_pay_multiplier_score else 0
        )
        results.personal_finances_and_pay_multiplier = round(finances_pay_multiplier_percentage, 2)

    # Barriers calculation
        barriers_questions = ['q189', 'q190', 'q191', 'q192', 'q193', 'q194', 'q195', 'q196']
    ...
    # Emotional Wellbeing Multiplier calculation
        emotional_wellbeing_multiplier_score = total_emotional_wellbeing_score * q225_response
        max_emotional_wellbeing_multiplier_score = 124
        emotional_wellbeing_multiplier_percentage = (
            (emotional_wellbeing_multiplier_score / max_emotional_wellbeing_multiplier_score) * 100
            if max_emotional_wellbeing_multiplier_score else 0
        )
        results.emotional_wellbeing_multiplier = round(emotional_wellbeing_multiplier_percentage, 2)
    ...
    # Financial Position as a Barrier for Holidays calculation
        ...
        financial_position_barrier_percentage = (
            (financial_position_barrier_score / max_financial_position_barrier_score) * 100
            if max_financial_position_barrier_score else 0
        )
        results.financial_position_as_a_barrier_for_holidays = round(financial_position_barrier_percentage, 2)
        
    # Identity calculation
        identity_questions = ['q162', 'q163', 'q164', 'q165', 'q166', 'q169', 'q170', 'q171', 'q172', 'q173', 'q174', 'q175']
        ...
        identity_percentage = (
            ...
            if max_identity_score else 0
        )
        results.identity = round(identity_percentage, 2)
    
    # Identity and Management Support calculation
        if total_identity_score > 0:
            identity_management_support_score = total_identity_score + total_management_support_score
        else:
            identity_management_support_score = 0
        max_identity_management_support_score = 75
        identity_management_support_percentage = (
            (identity_management_support_score / max_identity_management_support_score) * 100
            if max_identity_management_support_score else 0
        )
        results.identity_and_management_support = round(identity_management_support_percentage, 2)

    # Discrimination calculation
        discrimination_questions = ['q169', 'q170', 'q171', 'q172', 'q173', 'q174', 'q175']
        max_discrimination_score = 15
        total_discrimination_score = sum(
            int(form_data.get(question_key, 0)) for question_key in discrimination_questions
        )
        discrimination_percentage = (
            (total_discrimination_score / max_discrimination_score) * 100
            if max_discrimination_score else 0
        )
    ...
    # Discrimination Work Impact calculation
        if total_discrimination_score > 0:
            discrimination_work_impact_score = total_discrimination_score * q225_response
        else:
            discrimination_work_impact_score = 0
        max_discrimination_work_impact_score = 60
        discrimination_work_impact_percentage = (
            (discrimination_work_impact_score / max_discrimination_work_impact_score) * 100
            if max_discrimination_work_impact_score else 0
        )
        results.discrimination_work_impact = round(discrimination_work_impact_percentage, 2)
    ...
    # Personal Relationships calculation
        ...
        total_personal_relationships_score = sum(
            int(form_data.get(question_key, 0)) for question_key in personal_relationships_questions
        )
        personal_relationships_percentage = (
            (total_personal_relationships_score / max_personal_relationships_score) * 100
            if max_personal_relationships_score else 0
        )
    ...
    # Personal Relationships and Working Relationships Impact calculation
        if total_personal_relationships_score > 0:
            personal_work_relationships_impact_score = total_personal_relationships_score + int(form_data.get('q179', 0))
        else:
        ...
        personal_work_relationships_impact_percentage = (
            (personal_work_relationships_impact_score / max_personal_work_relationships_impact_score) * 100
            if max_personal_work_relationships_impact_score else 0
        )
        results.personal_relationships_and_working_relationships_impact = round(personal_work_relationships_impact_percentage, 2)
    ...
    # Emotional Distress calculation
        ...
        total_emotional_distress_score = sum(
            int(form_data.get(question_key, 0)) for question_key in emotional_distress_questions
        )
        emotional_distress_percentage = (
            (total_emotional_distress_score / max_emotional_distress_score) * 100
            if max_emotional_distress_score else 0
        )
        results.emotional_distress = round(emotional_distress_percentage, 2)

    # Emotional Distress Multiplier calculation
        emotional_distress_multiplier_score = total_emotional_distress_score * q225_response
        max_emotional_distress_multiplier_score = 48
        emotional_distress_multiplier_percentage = (
            (emotional_distress_multiplier_score / max_emotional_distress_multiplier_score) * 100
            if max_emotional_distress_multiplier_score else 0
        )
        results.emotional_distress_multiplier = round(emotional_distress_multiplier_percentage, 2)
    ...
    # Holidays and Pay calculation
        ...
        holidays_and_pay_percentage = (
            ...
            if max_holidays_and_pay_score else 0
        )
    ...
    # Emotional Wellbeing and Management Support calculation
        if total_emotional_wellbeing_score > 0:
            wellbeing_management_support_score = total_emotional_wellbeing_score + total_management_support_score
        else:
            wellbeing_management_support_score = 0
        max_wellbeing_management_support_score = 67
        wellbeing_management_support_percentage = (
            (wellbeing_management_support_score / max_wellbeing_management_support_score) * 100
            if max_wellbeing_management_support_score else 0
        )
        results.emotional_wellbeing_and_management_support = round(wellbeing_management_support_percentage, 2)
    ...
    # Personal Multiplier calculation
        ...
        personal_multiplier_percentage = (
            ...
            if max_personal_multiplier_score else 0
        )
    ...
    # Mental and Physical Health and Absence calculation
        if mental_physical_health_score > 0:
            mental_physical_absence_score = mental_physical_health_score + absence_and_support_score
        else:
        ...
        mental_physical_absence_percentage = (
            ...
            if max_mental_physical_absence_score else 0
        )
        results.mental_and_physical_health_and_absence_from_work = round(mental_physical_absence_percentage, 2)
        ...
        mental_health_culture_percentage = (
            ...
            if max_mental_health_culture_score else 0
        )
        results.mental_health_and_culture_multiplier = round(mental_health_culture_percentage, 2)
        ...
        health_and_safety_multiplier_percentage = (
            (health_and_safety_multiplier_score / max_health_and_safety_multiplier_score) * 100
            if max_health_and_safety_multiplier_score else 0
        )
        results.health_and_safety_multiplier = round(health_and_safety_multiplier_percentage, 2)

        # Fertility and Pregnancy calculation
        fertility_questions = ['q123', 'q124', 'q125', 'q126', 'q127', 'q128', 'q129', 'q130', 'q131', 'q132']
        ...
        abuse_trauma_impact_percentage = (
            ...
            if max_abuse_trauma_impact_score else 0
        )
    ...
    # Abuse and Trauma and Management Support and Culture calculation
        if total_abuse_trauma_score > 0:
            abuse_trauma_management_culture_score = total_abuse_trauma_score + total_culture_score
        else:
        ...
        abuse_trauma_management_culture_percentage = (
            (abuse_trauma_management_culture_score / max_abuse_trauma_management_culture_score) * 100
            if max_abuse_trauma_management_culture_score else 0
        )
        results.abuse_and_trauma_and_management_support_and_culture = round(abuse_trauma_management_culture_percentage, 2)
        ...
        self_care_percentage = (
            ...
            if max_self_care_score else 0
        )
        results.self_care = round(self_care_percentage, 2)

        # Self-Care and Physical Health calculation
        if total_self_care_score > 0:
            self_care_physical_health_score = total_self_care_score + total_physical_health_score
        else:
            self_care_physical_health_score = 0
        max_self_care_physical_health_score = 46
        self_care_physical_health_percentage = (
            (self_care_physical_health_score / max_self_care_physical_health_score) * 100
            if max_self_care_physical_health_score else 0
        )
        results.self_care_and_physical_health = round(self_care_physical_health_percentage, 2)

        # Self-Care and Mental Health calculation
        if total_self_care_score > 0:
            self_care_mental_health_score = total_self_care_score + total_mental_health_score
        else:
        ...
        self_care_mental_health_percentage = (
            ...
            if max_self_care_mental_health_score else 0
        )
        results.self_care_and_mental_health = round(self_care_mental_health_percentage, 2)
            
        # Emotional Health and Culture at Work calculation
        if total_emotional_wellbeing_score > 0:
            emotional_health_culture_score = total_emotional_wellbeing_score + total_culture_score
        else:
        ...
        emotional_health_culture_percentage = (
            ...
            if max_emotional_health_culture_score else 0
        )
        results.emotional_health_and_culture_at_work = round(emotional_health_culture_percentage, 2)
        ...
        age_discrimination_percentage = (
            ...
            if max_age_discrimination_score else 0
        )
        ...
        race_discrimination_percentage = (
            ...
            if max_race_discrimination_score else 0
        )
        ...
        disability_discrimination_percentage = (
            (disability_discrimination_score / max_disability_discrimination_score) * 100
            if max_disability_discrimination_score else 0
        )
        results.disability_discrimination = round(disability_discrimination_percentage, 2)
        ...
        gender_discrimination_percentage = (
            ...
            if max_gender_discrimination_score else 0
        )
        ...
        sexual_discrimination_percentage = (
            ...
            if max_sexual_discrimination_score else 0
        )
        ...
        other_discrimination_percentage = (
            ...
            if max_other_discrimination_score else 0
        )
        results.other_discrimination = round(other_discrimination_percentage, 2)

        #abuse_and_trauma_and_management_support_multiplier
        if total_abuse_trauma_score > 0:
            abuse_trauma_management_support_multiplier_score = total_abuse_trauma_score + total_management_support_score
        else:
        ...
        abuse_trauma_management_support_multiplier_score_percentage = (
            (abuse_trauma_management_support_multiplier_score / max_abuse_trauma_management_support_multiplier_score) * 100
            if max_abuse_trauma_management_support_multiplier_score else 0
        )
        results.abuse_and_trauma_and_management_support_multiplier = round(abuse_trauma_management_support_multiplier_score_percentage, 2)

        #Emotional health multiplier
        emotional_health_multiplier_score = int(form_data.get('q225', 0)) * total_emotional_health_score
        max_emotional_health_multiplier_score = 40
        emotional_health_multiplier_percentage = (
            (emotional_health_multiplier_score / max_emotional_health_multiplier_score) * 100
            if max_emotional_health_multiplier_score else 0
        )
        results.emotional_health_multiplier = round(emotional_health_multiplier_percentage, 2)
        ...
        total_responsibility_family_score = sum(
            int(form_data.get(question_key, 0)) for question_key in responsibility_family
        )
        responsibility_family_percentage = (total_responsibility_family_score / max_responsibility_family_score) * 100
        ...
        if total_responsibility_children_score + total_responsibility_family_score > 0:
            family_factors_score = total_responsibility_children_score + childcare_multiplier_score + total_responsibility_family_score
        else:
        ...
        family_factors_score_percentage = (
            ...
            if max_family_factors_score else 0
        )
        ...
        family_multiplier_percentage = (
            ...
            if max_family_multiplier_score else 0
        )
        ...
        fertility_pregnancy_work_percentage = (
            ...
            if max_fertility_pregnancy_work_score else 0
        )
        results.fertility_and_pregnancy_impacting_work = round(fertility_pregnancy_work_percentage, 2)
        ...
        work_commitment_barrier_for_holidays_percentage = (
            (work_commitment_barrier_for_holidays_score / max_work_commitment_barrier_for_holidays_score) * 100
            if max_work_commitment_barrier_for_holidays_score else 0
        )
        results.work_commitments_as_a_barrier_for_holidays = round(work_commitment_barrier_for_holidays_percentage, 2)
        ...
        physical_health_factors_impacting_work_percentage = (
            (physical_health_factors_impacting_work_score / max_physical_health_factors_impacting_work_score) * 100
            if max_physical_health_factors_impacting_work_score else 0
        )
        results.physical_health_factors_impacting_work = round(physical_health_factors_impacting_work_percentage, 2)
        ...
        physical_health_condition_affecting_work_percentage = (
            (physical_health_condition_affecting_work_score / max_physical_health_condition_affecting_work_score) * 100
            if max_physical_health_condition_affecting_work_score else 0
        )
        results.physical_health_condition_affecting_work = round(physical_health_condition_affecting_work_percentage, 2)
        ...
        mental_health_factors_impacting_work_percentage = (
            (mental_health_factors_impacting_work_score / max_mental_health_factors_impacting_work_score) * 100
            if max_mental_health_factors_impacting_work_score else 0
        )
        results.mental_health_factors_impacting_work = round(mental_health_factors_impacting_work_percentage, 2)
        ...
        max_hobbies_score = 2
        hobbies_percentage = (
            (hobbies_score / max_hobbies_score) * 100
            if max_hobbies_score else 0
        )
        ...
        if total_mental_health_score + total_physical_health_score > 0:
            mental_physical_absence_from_work_score = total_mental_health_score + total_physical_health_score + absence_and_support_score
        else:
        ...
        mental_physical_absence_from_work_percentage = (
            (mental_physical_absence_from_work_score / max_mental_physical_absence_from_work_score) * 100
            if max_mental_physical_absence_from_work_score else 0
        )
        results.mental_and_physical_health_and_absence_from_work = round(mental_physical_absence_from_work_percentage, 2)

        #Mental health and absence multiplier
        mental_health_absence_multiplier_score = (total_mental_health_score + total_absence_score) * q121_response
        max_mental_health_absence_multiplier_score = 168
        mental_health_absence_multiplier_percentage = (
            (mental_health_absence_multiplier_score / max_mental_health_absence_multiplier_score) * 100
            if max_mental_health_absence_multiplier_score else 0
        )
        results.mental_health_and_absence_multiplier = round(mental_health_absence_multiplier_percentage, 2)

        #Mental health, physical health and culture multiplier
        if total_mental_health_score + total_physical_health_score > 0:
            mental_physical_culture_multiplier_score = total_mental_health_score + total_physical_health_score + total_culture_score
        else:
        ...
        mental_physical_culture_multiplier_percentage = (
            (mental_physical_culture_multiplier_score / max_mental_physical_culture_multiplier_score) * 100
            if max_mental_physical_culture_multiplier_score else 0
        )
        results.mental_health_physical_health_and_culture_multiplier = round(mental_physical_culture_multiplier_percentage, 2)

        #Personal finances and pay, and childcare multiplier
        personal_finances_pay_childcare_multiplier_score = total_personal_finances_score * pay_childcare_multiplier_score
        max_personal_finances_pay_childcare_multiplier_score = 1536
        personal_finances_pay_childcare_multiplier_percentage = (
            (personal_finances_pay_childcare_multiplier_score / max_personal_finances_pay_childcare_multiplier_score) * 100
            if max_personal_finances_pay_childcare_multiplier_score else 0
        )
        results.personal_finances_and_pay_and_childcare_multiplier = round(personal_finances_pay_childcare_multiplier_percentage, 2)

        #Personal relationships and working relationship impact
        if total_personal_relationships_score > 0:
            personal_working_relationship_impact_score = total_personal_relationships_score + int(form_data.get('q179', 0))
        else:
        ...
        personal_working_relationship_impact_percentage = (
            (personal_working_relationship_impact_score / max_personal_working_relationship_impact_score) * 100
            if max_personal_working_relationship_impact_score else 0
        )
        results.personal_relationships_and_working_relationships_impact = round(personal_working_relationship_impact_percentage, 2)

        #Physical health and culture at work
        physical_health_and_culture_work_score = (total_physical_health_score + total_culture_score) * q63_response
        max_physical_health_and_culture_work_score = 208
        physical_health_and_culture_work_percentage = (
            (physical_health_and_culture_work_score / max_physical_health_and_culture_work_score) * 100
            if max_physical_health_and_culture_work_score else 0
        )
        results.physical_health_and_culture = round(physical_health_and_culture_work_percentage, 2)

        #Physical health and management support multiplier
        physical_health_and_management_support_multiplier_score = (total_physical_health_score + total_management_support_score) * q63_response
        ...
        physical_health_and_management_support_multiplier_percentage = (
            (physical_health_and_management_support_multiplier_score / max_physical_health_and_management_support_multiplier_score) * 100
            if max_physical_health_and_management_support_multiplier_score else 0
        )
        results.physical_health_and_management_support_multiplier = round(physical_health_and_management_support_multiplier_percentage, 2)

        #Pregnancy and management support
        if pregnancy_impact_score > 0:
            pregnancy_management_support_score = pregnancy_impact_score + total_management_support_score
        else:
            pregnancy_management_support_score = 0
        max_pregnancy_management_support_score = 66
        pregnancy_management_support_percentage = (
            (pregnancy_management_support_score / max_pregnancy_management_support_score) * 100
            if max_pregnancy_management_support_score else 0
        )
        results.pregnancy_and_management_support = round(pregnancy_management_support_percentage, 2)

        #Pregnancy and physical health
        if pregnancy_impact_score > 0:
            pregnancy_physical_health_score = pregnancy_impact_score + total_physical_health_score
        else:
            pregnancy_physical_health_score = 0
        max_pregnancy_physical_health_score = 58
        pregnancy_physical_health_percentage = (
            (pregnancy_physical_health_score / max_pregnancy_physical_health_score) * 100
            if max_pregnancy_physical_health_score else 0
        )
        results.pregnancy_and_physical_health = round(pregnancy_physical_health_percentage, 2)

        #Pregnancy and mental health
        if pregnancy_impact_score > 0:
            pregnancy_mental_health_score = pregnancy_impact_score + total_mental_health_score
        else:
        ...
        pregnancy_mental_health_percentage = (
            ...
            if max_pregnancy_mental_health_score else 0
        )
        results.pregnancy_and_mental_health = round(pregnancy_mental_health_percentage, 2)
        ...
        support_network_percentage = (total_support_network_score / max_support_network_score) * 100
        ...
        support_network_multiplier_percentage = (support_network_multiplier_score / max_support_network_multiplier_score) * 100
        results.support_network_multiplier = round(support_network_multiplier_percentage, 2)
        ...
        training_multiplier_percentage = (
            ...
            if max_training_multiplier_score else 0
        )
...
def user_results(request):
    ...
    for key, value in results.items():
        ...
        if pres:
            texts[key] = {'content': report_text[key][cat]['individual'], 'category': cat }
            texts[f"{key}_overview"] = report_text[key]['overview']

    res = read_result(results)
    
    return render(request, 'bessie/result.html', {
...
def view_results(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            return render(request, 'bessie/error.html', {'message': 'this user has not taken the quiz yet'})
...
def calculate_stress_load(factors):
    ...
    for factor in factors:
        if isinstance(factor, dict) and "val" in factor:
            ...
            total += sum(int(value) for value in val.values() if isinstance(value, int) or str(value).isdigit())
...
def view_company_results(request, id):
    ...
    result = BessieResult.objects.filter(query).aggregate(
        ...
        control_and_autonomy_over_working_hours = Avg('control_and_autonomy_over_working_hours', default=0),
        ...
        complexity_training_hours_and_flexibility = Avg('complexity_training_hours_and_flexibility', default=0),
        ...
        team_and_colleague_support_multiplier = Avg('team_and_colleague_support_multiplier', default=0),
        ...
        lone_working_and_wider_team_support_multiplier = Avg('lone_working_and_wider_team_support_multiplier', default=0),
        ...
        work_related_stress_impacting_family_and_relationships = Avg('work_related_stress_impacting_family_and_relationships', default=0),
        environment_multiplier = Avg('environment_multiplier', default=0),
        work_related_stress_impacting_social_life = Avg('work_related_stress_impacting_social_life', default=0),
        work_related_stress_impacting_personal_life = Avg('work_related_stress_impacting_personal_life', default=0),
        ...
        responsibility_for_children_multiplier = Avg('responsibility_for_children_multiplier', default=0),
        ...
        physical_health_condition_affecting_work = Avg('physical_health_condition_affecting_work', default=0),
        physical_health_factors_impacting_work = Avg('physical_health_factors_impacting_work', default=0),
        mental_health_factors_impacting_work = Avg('mental_health_factors_impacting_work', default=0),
        physical_health = Avg('physical_health', default=0),
        physical_health_multiplier = Avg('physical_health_multiplier', default=0),
        physical_health_and_absence_multiplier = Avg('physical_health_and_absence_multiplier', default=0),
        physical_health_and_management_support_multiplier = Avg('physical_health_and_management_support_multiplier', default=0),
        ...
        mental_and_physical_health_multiplier = Avg('mental_and_physical_health_multiplier', default=0),
        mental_and_physical_health_and_absence_from_work = Avg('mental_and_physical_health_and_absence_from_work', default=0),
        mental_health_and_absence_multiplier = Avg('mental_health_and_absence_multiplier', default=0),
        mental_health_and_culture_multiplier = Avg('mental_health_and_culture_multiplier', default=0),
        mental_health_physical_health_and_culture_multiplier = Avg('mental_health_physical_health_and_culture_multiplier', default=0),
        ...
        fertility_and_pregnancy_impacting_work = Avg('fertility_and_pregnancy_impacting_work', default=0),
        ...
        abuse_and_trauma_and_management_support_multiplier = Avg('abuse_and_trauma_and_management_support_multiplier', default=0),
        abuse_and_trauma_and_management_support_and_culture = Avg('abuse_and_trauma_and_management_support_and_culture', default=0),
        ...
        personal_relationships_and_working_relationships_impact = Avg('personal_relationships_and_working_relationships_impact', default=0),
        problem_solving = Avg('problem_solving', default=0),
        personal_finances = Avg('personal_finances', default=0),
        personal_finances_and_responsibility_for_children = Avg('personal_finances_and_responsibility_for_children', default=0),
        personal_finances_and_pay_multiplier = Avg('personal_finances_and_pay_multiplier', default=0),
        personal_finances_and_pay_and_childcare_multiplier = Avg('personal_finances_and_pay_and_childcare_multiplier', default=0),
        hobbies = Avg('hobbies', default=0),
        personal_barriers = Avg('personal_barriers', default=0),
        financial_position_as_a_barrier_for_holidays = Avg('financial_position_as_a_barrier_for_holidays', default=0),
        work_commitments_as_a_barrier_for_holidays = Avg('work_commitments_as_a_barrier_for_holidays', default=0),
        ...
        personal_satisfaction_improvement = Avg('personal_satisfaction_improvement', default=0),
        ...
        emotional_wellbeing_and_management_support = Avg('emotional_wellbeing_and_management_support', default=0),
        emotional_health_and_culture_at_work = Avg('emotional_health_and_culture_at_work', default=0),
    ...
    return render(request, 'bessie/company_results.html', {
            'stress_and_wellbeing': json.dumps(res['stress_and_wellbeing']),
            'stress_risks_affecting_work': json.dumps(res['stress_risks_affecting_work']),
            'workplace_stress_factors': json.dumps(res['workplace_stress_factors']),
            'presenteeism_factors': json.dumps(res['presenteeism']),
...
def user_list(request, id):
    ...
    return render(request, 'bessie/user_list.html', {
            "company": company,
            "page_obj": page_obj
        })
...
def read_result(res: BessieResult):
    ...
    stress_risks_affecting_work['abuse_and_trauma_and_management_support_multiplier'] = res['abuse_and_trauma_and_management_support_multiplier']
    ...
    stress_risks_affecting_work['emotional_distress_multiplier'] = res['emotional_distress_multiplier']
    stress_risks_affecting_work['emotional_health_multiplier'] = res['emotional_health_multiplier']
    stress_risks_affecting_work['emotional_wellbeing_multiplier'] = res['emotional_wellbeing_multiplier']
    ...
    stress_risks_affecting_work['health_and_safety_multiplier'] = res['health_and_safety_multiplier']
    stress_risks_affecting_work['lone_working_and_wider_team_support_multiplier'] = res['lone_working_and_wider_team_support_multiplier']
    stress_risks_affecting_work['management_support_multiplier'] = res['management_support_multiplier']
    stress_risks_affecting_work['mental_and_physical_health_multiplier'] = res['mental_and_physical_health_multiplier']
    stress_risks_affecting_work['mental_health_and_absence_multiplier'] = res['mental_health_and_absence_multiplier']
    stress_risks_affecting_work['mental_health_and_culture_multiplier'] = res['mental_health_and_culture_multiplier']
    stress_risks_affecting_work['mental_health_multiplier'] = res['mental_health_multiplier']
    stress_risks_affecting_work['mental_health_physical_health_and_culture_multiplier'] = res['mental_health_physical_health_and_culture_multiplier']
    stress_risks_affecting_work['pay_and_childcare_multiplier'] = res['pay_and_childcare_multiplier']
    stress_risks_affecting_work['pay_multiplier'] = res['pay_multiplier']
    stress_risks_affecting_work['personal_finances_and_pay_and_childcare_multiplier'] = res['personal_finances_and_pay_and_childcare_multiplier']
    stress_risks_affecting_work['personal_finances_and_pay_multiplier'] = res['personal_finances_and_pay_multiplier']
    stress_risks_affecting_work['personal_multiplier'] = res['personal_multiplier']
    stress_risks_affecting_work['physical_health_and_absence_multiplier'] = res['physical_health_and_absence_multiplier']
    stress_risks_affecting_work['physical_health_and_management_support_multiplier'] = res['physical_health_and_management_support_multiplier']
    stress_risks_affecting_work['physical_health_multiplier'] = res['physical_health_multiplier']
    stress_risks_affecting_work['privacy_multiplier'] = res['privacy_multiplier']
    stress_risks_affecting_work['responsibility_for_children_multiplier'] = res['responsibility_for_children_multiplier']
    stress_risks_affecting_work['support_network_multiplier'] = res['support_network_multiplier']
    stress_risks_affecting_work['team_and_colleague_support'] = res['team_and_colleague_support']
    ...
    environment['complexity_training_hours_and_flexibility'] = res['complexity_training_hours_and_flexibility']
    environment['complexity_plus_training'] = res['complexity_plus_training']
    environment['consultation'] = res['consultation']
    environment['control_and_autonomy_over_working_hours'] = res['control_and_autonomy_over_working_hours']
    ...
    environment['lone_working_and_wider_team_support_multiplier'] = res['lone_working_and_wider_team_support_multiplier']
    ...
    environment['sick_leave_and_employer_support'] = res['sick_leave_and_employer_support']
    environment['team_and_colleague_support'] = res['team_and_colleague_support']
    environment['team_and_colleague_support_multiplier'] = res['team_and_colleague_support_multiplier']
    ...
    environment['work_related_stress_impacting_family_and_relationships'] = res['work_related_stress_impacting_family_and_relationships']
    environment['work_related_stress_impacting_personal_life'] = res['work_related_stress_impacting_personal_life']
    environment['work_related_stress_impacting_social_life'] = res['work_related_stress_impacting_social_life']
    environment['workload'] = res['workload']
    environment['manageable_workload'] = res['manageable_workload']

    presenteeism['control_and_autonomy_over_working_hours'] = res['control_and_autonomy_over_working_hours']
    presenteeism['fertility_and_pregnancy_impacting_work'] = res['fertility_and_pregnancy_impacting_work']
    presenteeism['financial_position_as_a_barrier_for_holidays'] = res['financial_position_as_a_barrier_for_holidays']
    ...
    presenteeism['mental_health_factors_impacting_work'] = res['mental_health_factors_impacting_work']
    presenteeism['overtime'] = res['overtime']
    presenteeism['physical_health'] = res['physical_health']
    presenteeism['physical_health_factors_impacting_work'] = res['physical_health_factors_impacting_work']
    presenteeism['sick_leave_and_employer_support'] = res['sick_leave_and_employer_support']
    presenteeism['work_breaks'] = res['work_breaks']
    presenteeism['work_commitments_as_a_barrier_for_holidays'] = res['work_commitments_as_a_barrier_for_holidays']
    ...
    family['responsibility_for_children_multiplier'] = res['responsibility_for_children_multiplier']
    ...
    health['abuse_and_trauma_and_management_support_and_culture'] = res['abuse_and_trauma_and_management_support_and_culture']
    health['abuse_and_trauma_and_management_support_multiplier'] = res['abuse_and_trauma_and_management_support_multiplier']
    health['abuse_and_trauma_impact'] = res['abuse_and_trauma_impact']
    health['fertility_and_pregnancy'] = res['fertility_and_pregnancy']
    health['fertility_and_pregnancy_impacting_work'] = res['fertility_and_pregnancy_impacting_work']
    health['health'] = res['health']
    health['mental_and_physical_health'] = res['mental_and_physical_health']
    health['mental_and_physical_health_and_absence_from_work'] = res['mental_and_physical_health_and_absence_from_work']
    health['mental_and_physical_health_multiplier'] = res['mental_and_physical_health_multiplier']
    health['mental_health'] = res['mental_health']
    health['mental_health_and_absence_multiplier'] = res['mental_health_and_absence_multiplier']
    health['mental_health_and_culture_multiplier'] = res['mental_health_and_culture_multiplier']
    health['mental_health_factors_impacting_work'] = res['mental_health_factors_impacting_work']
    health['mental_health_multiplier'] = res['mental_health_multiplier']
    health['mental_health_physical_health_and_culture_multiplier'] = res['mental_health_physical_health_and_culture_multiplier']
    health['physical_health'] = res['physical_health']
    health['physical_health_and_absence_multiplier'] = res['physical_health_and_absence_multiplier']
    health['physical_health_and_culture'] = res['physical_health_and_culture']
    health['physical_health_and_management_support_multiplier'] = res['physical_health_and_management_support_multiplier']
    health['physical_health_condition_affecting_work'] = res['physical_health_condition_affecting_work']
    ...
    personal['emotional_health_and_culture_at_work'] = res['emotional_health_and_culture_at_work']
    personal['emotional_health_multiplier'] = res['emotional_health_multiplier']
    personal['emotional_wellbeing'] = res['emotional_wellbeing']
    personal['emotional_wellbeing_and_management_support'] = res['emotional_wellbeing_and_management_support']
    personal['emotional_wellbeing_multiplier'] = res['emotional_wellbeing_multiplier']
    personal['financial_position_as_a_barrier_for_holidays'] = res['financial_position_as_a_barrier_for_holidays']
    ...
    personal['personal_finances_and_pay_and_childcare_multiplier'] = res['personal_finances_and_pay_and_childcare_multiplier']
    personal['personal_finances_and_pay_multiplier'] = res['personal_finances_and_pay_multiplier']
    personal['personal_finances_and_responsibility_for_children'] = res['personal_finances_and_responsibility_for_children']
    personal['personal_multiplier'] = res['personal_multiplier']
    personal['personal_relationships'] = res['personal_relationships']
    personal['personal_relationships_and_working_relationships_impact'] = res['personal_relationships_and_working_relationships_impact']
    personal['personal_satisfaction'] = res['personal_satisfaction']
    personal['personal_satisfaction_improvement'] = res['personal_satisfaction_improvement']
    ...
    personal['work_commitments_as_a_barrier_for_holidays'] = res['work_commitments_as_a_barrier_for_holidays']

    return {
            'stress_and_wellbeing': [{'attr': key,  'val': value} for key, value in stress_and_wellbeing.items()],
            'workplace_stress_factors': [{'attr': key,  'val': value} for key, value in workplace_stress_factors.items()] ,
            'stress_risks_affecting_work': [{'attr': key,  'val': value} for key, value in stress_risks_affecting_work.items()],
            'presenteeism': [{'attr': key,  'val': value} for key, value in presenteeism.items()],
...
def get_field_statistics(companyId, team=None):
    ...
    if team:
        query &= Q(response__employee__team=team)
    fields = [
        'environment', 'residence', 'privacy', 'privacy_multiplier',
        'travel_to_work', 'pay', 'pay_and_childcare_multiplier','pay_multiplier', 'complexity', 'hours_and_flexibility',
        'control_and_autonomy_over_working_hours', 'workload','manageable_workload', 'complexity_plus_training',
        'complexity_training_hours_and_flexibility', 'work_breaks','overtime','team_and_colleague_support',
        'management_support','management_support_multiplier','team_and_colleague_support_multiplier','training',
        'training_multiplier','culture','managerial_trust','colleague_trust','consultation','team_morale',
        'willingness_to_progress','culture_multiplier','lone_working','wider_team_support','lone_working_and_wider_team_support_multiplier',
        'health_and_safety','health_and_safety_multiplier','absence','sick_leave_and_employer_support','absence_and_support',
        'absence_multiplier','covid','covid_multiplier','work_related_stress_impacting_family_and_relationships','environment_multiplier',
        'work_related_stress_impacting_social_life','work_related_stress_impacting_personal_life','family','family_multiplier',
        'family_support_companion','responsibility_of_children','responsibility_for_children_multiplier','support_network',
        'support_network_multiplier','childcare','childcare_multiplier','responsibility_for_family','family_factors','health',
        'physical_health_condition_affecting_work','physical_health_factors_impacting_work','mental_health_factors_impacting_work','physical_health',
        'physical_health_multiplier','physical_health_and_absence_multiplier','physical_health_and_management_support_multiplier','physical_health_and_culture',
        'mental_health','mental_and_physical_health','mental_and_physical_health_multiplier','mental_and_physical_health_and_absence_from_work','mental_health_and_absence_multiplier',
        'mental_health_and_culture_multiplier','mental_health_physical_health_and_culture_multiplier','mental_health_multiplier','fertility_and_pregnancy',
        'fertility_and_pregnancy_impacting_work','pregnancy_impact','pregnancy_and_management_support','abuse_and_trauma',
        'abuse_and_trauma_impact','abuse_and_trauma_and_management_support_multiplier','abuse_and_trauma_and_management_support_and_culture','pregnancy_and_mental_health',
        'pregnancy_and_physical_health','self_care','self_care_and_physical_health',
        'self_care_and_mental_health','personal','identity','identity_and_management_support','discrimination','discrimination_work_impact','age_discrimination',
        'race_discrimination','disability_discrimination','gender_discrimination','sexual_discrimination','other_discrimination','carer',
        'personal_relationships','personal_relationships_and_working_relationships_impact','problem_solving','personal_finances',
        'personal_finances_and_responsibility_for_children','personal_finances_and_pay_multiplier','personal_finances_and_pay_and_childcare_multiplier','hobbies',
        'personal_barriers','financial_position_as_a_barrier_for_holidays','work_commitments_as_a_barrier_for_holidays','holidays',
        'holidays_and_pay','personal_satisfaction','personal_satisfaction_improvement','emotional_health',
        'emotional_health_multiplier','emotional_distress','emotional_distress_multiplier','emotional_wellbeing','emotional_wellbeing_and_management_support',
        'emotional_health_and_culture_at_work','personal_multiplier','emotional_wellbeing_multiplier'
        ]
    ...
    for field in fields:
        stats = BessieResult.objects.filter(query).annotate(
            field_name=Value(field, output_field=CharField())
        ).values('field_name').annotate(
            low_count=Count(Case(
                When(**{f"{field}__lte": 25.0}, then=1),
                output_field=IntegerField(),
            )),
            medium_count=Count(Case(
                When(**{f"{field}__gt": 25.0, f"{field}__lte": 50.0}, then=1),
                output_field=IntegerField(),
            )),
            high_count=Count(Case(
                When(**{f"{field}__gt": 50.0, f"{field}__lte": 75.0}, then=1),
                output_field=IntegerField(),
            )),
            very_high_count=Count(Case(
                When(**{f"{field}__gt": 75.0}, then=1),
                output_field=IntegerField(),
            ))
        )
        result[field] =  {'low_count': stats[0]['low_count'], 'medium_count': stats[0]['medium_count'], 'high_count': stats[0]['high_count'], 'very_high': stats[0]['very_high_count']}
